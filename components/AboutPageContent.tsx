...
const defaultData: IAboutData = {
	hero: {
		...
		subtitle: "মারকাজুত তাহফিজ ইন্সটিটিউশনাল মাদরাসার ইতিহাস ও মিশন",
		backgroundImage: "",
	},
	mission: {
		title: "মিশন:",
		content: "ইসলামী শিক্ষার মাধ্যমে দেশের যুব সমাজকে সুশিক্ষিত...",
	},
	vision: { title: "ভিশন:", content: "একটি বিশ্বমানের শিক্ষা প্রতিষ্ঠান..." },
	history: {
		title: "প্রতিষ্ঠানের ইতিহাস",
		paragraphs: ["মারকাজুত তাহফিজ ইন্সটিটিউশনাল মাদরাসা..."],
	},
	features: [],
	achievements: [],
	programs: [],
	...
	cta: {
		...
		description: "আপনার সন্তানের সর্বোত্তম ভবিষ্যতের জন্য...",
		buttonText: "এখনই ভর্তি আবেদন করুন",
	},
};
		...
		setData((prev) => {
			if (subKey) {
				...
				return {
					...
					[section]: {
						...sectionObj,
						[subKey]: (sectionObj[subKey] as any[]).filter(
							(_, i) => i !== index
						),
					},
				};
		...
		<div className="space-y-8 pb-10">
					...
					<div className="space-y-4">
						...
						<div className="space-y-2">
							...
							<Textarea
								...
								onChange={(e) => update("mission", "content", e.target.value)}
							/>
						</div>
					</div>
					<div className="space-y-4">
						...
						<div className="space-y-2">
							...
							<Textarea
								...
								onChange={(e) => update("vision", "content", e.target.value)}
							/>
						</div>
					</div>
				</div>
			...
			<div className="bg-white p-6 rounded-lg shadow-sm border space-y-4">
				...
				<div className="space-y-3">
					{data.history.paragraphs.map((p, i) => (
						<div key={i} className="flex gap-2 items-start">
							<Textarea
								...
								value={p}
								onChange={(e) =>
									update("history", "paragraphs", e.target.value, i)
								}
								placeholder={`প্যারাগ্রাফ ${i + 1}`}
							/>
							<Button
								variant="destructive"
								size="icon"
								onClick={() => removeArrayItem("history", i, "paragraphs")}
							>
					...
					<Button
						variant="outline"
						size="sm"
						onClick={() => addArrayItem("history", "", "paragraphs")}
					>
						<Plus className="w-4 h-4 mr-2" /> প্যারাগ্রাফ যোগ করুন
					</Button>
				</div>
			</div>
			...
			<div className="bg-white p-6 rounded-lg shadow-sm border space-y-4">
					...
					{data.features.map((item, i) => (
						<div key={i} className="border p-4 rounded-md space-y-3 relative">
							...
							<Textarea
								...
								onChange={(e) =>
									update("features", null, e.target.value, i, "description")
								}
							/>
						</div>
					))}
				...
				<Button
					variant="outline"
					onClick={() =>
						addArrayItem("features", { title: "", description: "", icon: "" })
					}
				>
					<Plus className="w-4 h-4 mr-2" /> বৈশিষ্ট্য যোগ করুন
				</Button>
			</div>
			...
			<div className="bg-white p-6 rounded-lg shadow-sm border space-y-4">
					...
					{data.achievements.map((item, i) => (
						<div key={i} className="border p-4 rounded-md space-y-2 relative">
							...
							<Input
								value={item.title}
								onChange={(e) =>
									update("achievements", null, e.target.value, i, "title")
								}
							/>
							...
							<Input
								value={item.description}
								onChange={(e) =>
									update("achievements", null, e.target.value, i, "description")
								}
							/>
						</div>
					))}
				...
				<Button
					variant="outline"
					onClick={() =>
						addArrayItem("achievements", { title: "", description: "" })
					}
				>
					<Plus className="w-4 h-4 mr-2" /> অর্জন যোগ করুন
				</Button>
			</div>
			...
			<div className="bg-white p-6 rounded-lg shadow-sm border space-y-4">
					...
					{data.programs.map((item, i) => (
						<div key={i} className="border p-4 rounded-md space-y-2 relative">
							<Button
								...
								onClick={() => removeArrayItem("programs", i)}
							>
								<Trash2 className="w-3 h-3" />
							</Button>
							<Input
								placeholder="প্রোগ্রাম নাম"
								value={item.title}
								onChange={(e) =>
									update("programs", null, e.target.value, i, "title")
								}
								className="font-bold"
							/>
							<Input
								placeholder="সময়কাল (৩ বছর)"
								value={item.duration}
								onChange={(e) =>
									update("programs", null, e.target.value, i, "duration")
								}
							/>
							<Textarea
								placeholder="বর্ণনা"
								value={item.description}
								onChange={(e) =>
									update("programs", null, e.target.value, i, "description")
								}
							/>
						</div>
					))}
				...
				>
					<Plus className="w-4 h-4 mr-2" /> প্রোগ্রাম যোগ করুন
				</Button>
			</div>
			...
			<div className="bg-white p-6 rounded-lg shadow-sm border space-y-4">
				...
				<div className="grid md:grid-cols-2 gap-4">
					{data.values.map((item, i) => (
						<div key={i} className="border p-4 rounded-md space-y-2 relative">
							...
							<Input
								placeholder="মূল্যবোধ"
								value={item.title}
								onChange={(e) =>
									update("values", null, e.target.value, i, "title")
								}
								className="font-bold"
							/>
							<Textarea
								...
								value={item.description}
								onChange={(e) =>
									update("values", null, e.target.value, i, "description")
								}
							/>
						</div>
					))}
				...
				>
					<Plus className="w-4 h-4 mr-2" /> মূল্যবোধ যোগ করুন
				</Button>
			</div>

			{/* Facilities - Dynamic as requested */}
			<div className="bg-white p-6 rounded-lg shadow-sm border space-y-4">
				<h2 className="text-xl font-semibold border-b pb-2 text-green-700">
					আমাদের সুবিধাসমূহ (Facilities)
				</h2>
				<div className="grid md:grid-cols-2 gap-4">
					{data.facilities.map((item, i) => (
						<div
							key={i}
							className="border p-4 rounded-md flex gap-3 items-start bg-gray-50"
						>
							<div className="w-8 h-8 bg-green-600 text-white rounded-full flex items-center justify-center shrink-0 font-bold mt-1">
								✓
							</div>
							<div className="flex-1 space-y-2">
								<Input
									placeholder="সুবিধার নাম"
									value={item.title}
									onChange={(e) =>
										update("facilities", null, e.target.value, i, "title")
									}
									className="font-bold border-green-200 focus:border-green-500"
								/>
								<Textarea
									placeholder="বিস্তারিত বর্ণনা"
									value={item.description}
									onChange={(e) =>
										update("facilities", null, e.target.value, i, "description")
									}
									className="text-sm"
									rows={2}
								/>
				...
				<Button
					variant="outline"
					onClick={() =>
						addArrayItem("facilities", { title: "", description: "" })
					}
					className="w-full border-green-200 hover:bg-green-50 text-green-700"
				>
					<Plus className="w-4 h-4 mr-2" /> নতুন সুবিধা যোগ করুন
				</Button>
			</div>
