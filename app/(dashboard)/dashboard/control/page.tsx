...
export default function ControlDashboardPage() {
  ...
  const refreshFinanceSummary = async () => {
    try {
      const res = await jsonFetch("/api/finance/summary?groupBy=month");
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {activeTab === "admission" && (
          <Card>
            <CardHeader>
              <CardTitle>অ্যাডমিশন অ্যাপ্লিকেশন</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {admissions.map((a) => (
                  <div key={a._id} className="border rounded-md p-3 space-y-1">
                    <div className="font-medium">{a.name}</div>
                    <div className="text-sm text-gray-600">
                      ক্লাস: {a.class} | ফোন: {a.phone}
                    </div>
                    <div className="text-xs text-gray-500">
                      স্ট্যাটাস: {a.status} |{" "}
                      ...
                      {["pending", "reviewing", "accepted", "rejected"].map(
                        (s) => (
                          <Button
                            ...
                            size="sm"
                            onClick={() => handleAdmissionStatus(a._id, s)}
                            disabled={loading}
                          >
                            {s}
                          </Button>
