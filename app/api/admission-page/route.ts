...
export async function POST(request: NextRequest) {
	try {
		...
		// Sanitize body by removing _id from subdocuments before saving to prevent Mongoose CastErrors
		if (body.classes && Array.isArray(body.classes)) {
			body.classes = body.classes.map((cls: any) => {
				const { _id, ...rest } = cls;
				return rest;
			});
		}
		if (body.faq && Array.isArray(body.faq)) {
			body.faq = body.faq.map((f: any) => {
				const { _id, ...rest } = f;
				return rest;
			});
		}
		if (body.infoCards && Array.isArray(body.infoCards)) {
			body.infoCards = body.infoCards.map((card: any) => {
				const { _id, ...rest } = card;
				return rest;
			});
		}
